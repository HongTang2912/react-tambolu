...
export default function Register() {
  ...
  const getValues = () => {
    loginUser(
      ...
      setLoginState(res.password === passwordField.current.childNodes[0].value)
      if (isSaveAccount)
        window.localStorage.setItem('saved-account', usernameField.current.childNodes[0].value)
      else 
        window.localStorage.setItem('saved-account', '')
    }).catch(err => {
      console.log(err)
    })
    setOpen(true);
        ...
        <div  className="h-screen flex items-center">
          ...
          >
            ...
            <Button 
              ...
              onClick={() => {getValues()}}
            >Đăng nhập</Button>
            <small className={`username-label text-right ${Styles["Montserrat-font"]}`}>
              Chưa có tài khoản? 
              <Link href="/register">
